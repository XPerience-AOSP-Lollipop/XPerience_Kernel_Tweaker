...
public class KSMActivity extends Activity implements Constants, ActivityThemeChangeInterface {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (new File(KSM_PAGESSHARED_PATH[ksm]).exists()) {
            t1.setText(Helpers.LeerUnaLinea(KSM_PAGESSHARED_PATH[ksm]));
            ist1 = true;
        } else {
        ...
        if (new File(KSM_PAGESSHARED_PATH[ksm]).exists()) {
            t2.setText(Helpers.LeerUnaLinea(KSM_PAGESUNSHERED_PATH[ksm]));
            ist2 = true;
        } else {
        ...
        if (new File(KSM_PAGESSHARING_PATH[ksm]).exists()) {
            t3.setText(Helpers.LeerUnaLinea(KSM_PAGESSHARING_PATH[ksm]));
            ist3 = true;
        } else {
        ...
        if (new File(KSM_PAGESVOLATILE_PATH[ksm]).exists()) {
            t4.setText(Helpers.LeerUnaLinea(KSM_PAGESVOLATILE_PATH[ksm]));
            ist4 = true;
        } else {
        ...
        if (new File(KSM_FULLSCANS_PATH[ksm]).exists()) {
            t5.setText(Helpers.LeerUnaLinea(KSM_FULLSCANS_PATH[ksm]));
            ist5 = true;
        } else {
        ...
        if (new File(KSM_SLEEP_TIMES_PATH[ksm]).exists()) {
            t6.setText(Helpers.LeerUnaLinea(KSM_SLEEP_TIMES_PATH[ksm]));
            ist6 = true;
        } else {
        ...
        if (new File(KSM_PAGESSCANNED_PATH[ksm]).exists()) {
            t7.setText(Helpers.LeerUnaLinea(KSM_PAGESSCANNED_PATH[ksm]));
            ist7 = true;
        } else {
        ...
        ((Button) findViewById(R.id.rst)).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                Runnable runnable = new Runnable() {
                    public void run() {
                        final String vlast = Helpers.LeerUnaLinea(ksmpath + "/run");
    ...
    protected Handler mCurHandler = new Handler() {
        public void handleMessage(Message msg) {
            if (ist1) t1.setText(Helpers.LeerUnaLinea(KSM_PAGESSHARED_PATH[ksm]));
            if (ist2) t2.setText(Helpers.LeerUnaLinea(KSM_PAGESUNSHERED_PATH[ksm]));
            if (ist3) t3.setText(Helpers.LeerUnaLinea(KSM_PAGESSHARING_PATH[ksm]));
            if (ist4) t4.setText(Helpers.LeerUnaLinea(KSM_PAGESVOLATILE_PATH[ksm]));
            if (ist5) t5.setText(Helpers.LeerUnaLinea(KSM_FULLSCANS_PATH[ksm]));
            if (ist6) t6.setText(Helpers.LeerUnaLinea(KSM_SLEEP_TIMES_PATH[ksm]));
            if (ist7) t7.setText(Helpers.LeerUnaLinea(KSM_PAGESSCANNED_PATH[ksm]));
        }
    };
