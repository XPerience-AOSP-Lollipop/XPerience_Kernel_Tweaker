...
public class ZramActivity extends Activity implements Constants, SeekBar.OnSeekBarChangeListener, ActivityThemeChangeInterface {
    ...
    private int getDiskSize() {
        return parseInt(Helpers.LeerUnaLinea(ZRAM_SIZE_PATH.replace("zram0", "zram" + curcpu)));
    }

    private int getCompressedDataSize() {
        return parseInt(Helpers.LeerUnaLinea(ZRAM_COMPR_PATH.replace("zram0", "zram" + curcpu)));
    }

    private int getOriginalDataSize() {
        return parseInt(Helpers.LeerUnaLinea(ZRAM_ORIG_PATH.replace("zram0", "zram" + curcpu)));
    }
    ...
    public void set_values() {
        ...
        if (new File(ZRAM_COMPR_PATH.replace("zram0", "zram" + curcpu)).exists()) {
            t1.setText(Helpers.ReadableByteCount(parseInt(Helpers.LeerUnaLinea(ZRAM_COMPR_PATH.replace("zram0", "zram" + curcpu)))));
            ist1 = true;
        }
        if (new File(ZRAM_ORIG_PATH.replace("zram0", "zram" + curcpu)).exists()) {
            t3.setText(Helpers.ReadableByteCount(parseInt(Helpers.LeerUnaLinea(ZRAM_ORIG_PATH.replace("zram0", "zram" + curcpu)))));
            ist3 = true;
        }
        if (new File(ZRAM_MEMTOT_PATH.replace("zram0", "zram" + curcpu)).exists()) {
            t5.setText(Helpers.ReadableByteCount(parseInt(Helpers.LeerUnaLinea(ZRAM_MEMTOT_PATH.replace("zram0", "zram" + curcpu)))));
            ist5 = true;
        }
        if (ist1 && ist3 && ist5) {
