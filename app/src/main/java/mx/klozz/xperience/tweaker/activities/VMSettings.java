...
public class VMSettings extends Activity implements Constants, AdapterView.OnItemClickListener, ActivityThemeChangeInterface {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (new File(DYNAMIC_DIRTY_WRITEBACK_PATH).exists()) {
            isdyn = Helpers.LeerUnaLinea(DYNAMIC_DIRTY_WRITEBACK_PATH).equals("1");
        }
        ...
        isdyn = (new File(DYNAMIC_DIRTY_WRITEBACK_PATH).exists() && Helpers.LeerUnaLinea(DYNAMIC_DIRTY_WRITEBACK_PATH).equals("1"));
        ...
        for (String aP : p) {
            try {
                if (aP != null) {
                    ...
                    if (testprop(pn)) props.add(new Prop(pn, Helpers.LeerUnaLinea(aP).trim()));
                }
            } catch (Exception e) {
            }
        }
